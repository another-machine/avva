...
function _updateHistogram(huehist: HTMLElement, bins: Float32Array): void {
  ...
  for (let i = 0; i < bars.length; i++) {
    (bars[i] as HTMLElement).style.height = `${2 + ((bins[i] ?? 0) / mx) * 32}px`;
  }
}
...
interface VideoMonitorRefs {
  ...
  sbAct: HTMLElement; sbBri: HTMLElement; sbActbg: HTMLElement;
  sbContrast: HTMLElement; sbSpread: HTMLElement; sbSat: HTMLElement;
  sbHi: HTMLElement; sbLo: HTMLElement; sbMass: HTMLElement; sbVmag: HTMLElement;
  sbMx: HTMLElement; sbVy: HTMLElement;
  svAct: HTMLElement; svBri: HTMLElement; svActbg: HTMLElement;
  svContrast: HTMLElement; svVy: HTMLElement; svMx: HTMLElement;
  sparkAct: HTMLCanvasElement; sparkActBg: HTMLCanvasElement;
  sparkBri: HTMLCanvasElement; sparkContrast: HTMLCanvasElement; sparkVy: HTMLCanvasElement;
  huemark: HTMLElement; huehist: HTMLElement;
  hueV: HTMLElement; hueN: HTMLElement;
  mFps: HTMLElement; mSrc: HTMLElement; mRes: HTMLElement;
}
...
function _buildVideoDOM(host: HTMLElement): VideoMonitorRefs {
  ...
  const sbBri = mk("div", "meter__fill"); const svBri = mk("span", "sig__val");
  const sbAct = mk("div", "meter__fill"); const svAct = mk("span", "sig__val");
  const sbActbg = mk("div", "meter__fill"); const svActbg = mk("span", "sig__val");
  const sbContrast = mk("div", "meter__fill"); const svContrast = mk("span", "sig__val");
  ...
  const meterRow = (lbl: string, fill: HTMLElement, val?: HTMLElement) => {
    ...
    const m = mk("div", "meter"); m.appendChild(fill); d.appendChild(m);
  ...
  const mxTrack = mk("div", "pos-track"); mxTrack.appendChild(sbMx); mxRow.appendChild(mxTrack);
  ...
  const vyTrack = mk("div", "pos-track"); vyTrack.appendChild(sbVy); vyRow.appendChild(vyTrack);
  ...
  host.append(
    synthHdr, keyRow, noteRow, sigQuality, div,
    sparksDiv, div2,
    sceneHdr,
    ...
    meterRow("VMG", sbVmag),
    mxRow, vyRow, div3,
    hueLabel, hueVal, huebar, huehist, div4,
    metaDiv,
  );

  return {
    sigDot, sigKeyLbl, sigNumeral, sigNotename, sigQuality,
    sbAct, sbBri, sbActbg, sbContrast, sbSpread, sbSat,
    sbHi, sbLo, sbMass, sbVmag, sbMx, sbVy,
    svAct, svBri, svActbg, svContrast, svVy, svMx,
    sparkAct, sparkActBg, sparkBri, sparkContrast, sparkVy,
    huemark, huehist, hueV, hueN, mFps, mSrc, mRes,
  };
}
...
function _paintVideoSignals(
  ...
  const vMag = Math.min(1, Math.sqrt(out.vmx * out.vmx + out.vmy * out.vmy) * 20);
...
export function mountVideoMonitor(host: HTMLElement): { onMsg(msg: TelemetryMsg): void } {
  ...
  function paint() {
    ...
    const { video: out, fps = 0, sourceLabel, resLabel, synth, histBins } = latest;
    ...
    refs.huemark.style.setProperty("--hue-marker-pos", `${(out.hue / 360) * 100}%`);
...
export function mountAudioMonitor(host: HTMLElement): { onMsg(msg: TelemetryMsg): void } {
  const chromaContainer = document.createElement("div");
  chromaContainer.className = "chroma";
  host.appendChild(chromaContainer);
    ...
    if (currentPalette) {
      ...
      const sorted = currentPalette.slots
        .map((slot, i) => {
          ...
          let h1 = currentPalette!.slotBoundaryHues[i + 1] ?? currentPalette!.slotBoundaryHues[0];
          if (h1 <= h0) h1 += 360;
  ...
  for (const k of ["harmony.root", "harmony.scale", "harmony.octave", "harmony.rootHue",
    "harmony.mode", "harmony.palette", "harmony.crossZone"] as const) {
  ...
  return {
    onMsg(msg) {
      if (msg.audio) {
        latestWeights = (msg.audio.slots ?? msg.audio.degrees) as Float32Array;
      }
    },
  };
}
