/**
 * Contract tests for the hue wheel in src/harmony/palette.ts.
 *
 * Run with `npm test`.
 *
 * These came over with the wheel when it was pulled back out of
 * `@amplib/hue-wheel`. The repetition semantics are the ones worth keeping —
 * "a value can appear twice, adjacent or apart, and the two cases mean
 * different things" is the whole shape of the API, and it is the part that
 * would quietly break if the wheel were ever reshaped around chords.
 *
 * The hue round-trip and non-uniformity checks are not here: they moved to
 * @amplib/color with the maths they cover.
 */

import { fromPerceptual } from "@amplib/color";
import { Palette } from "../src/harmony/palette";
import { parseSlotList } from "../src/harmony/parse-slot-list";
...
check("list splits on commas", parseSlotList("g1,g2,g3").join() === "g1,g2,g3");
...
check(
  "all-same collapses to one band",
  Palette.fromString("g,g,g").bands.length === 1
);
