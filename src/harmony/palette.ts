 ...
 *
 * A hue wheel divided into equal sectors. Briefly published as
 * `@amplib/hue-wheel`, and pulled back here because one consumer cannot tell a
 * general abstraction from a well-shaped one. `crossZone` defaulting to 0.15
 * and a neighbour's weight capping at 0.5 read as principled with only AVVA
 * calling them and arbitrary the moment something else does — so the wheel
 * stays here until a second machine wants it and can argue for its own
 * defaults. The colour maths underneath it had no such question hanging over
 * it and did go out, as `@amplib/color`.
 *
 * Nothing below is AVVA-specific: `T` is still opaque and the wheel still never
 * inspects a slot. `chord-palette.ts` remains the seam where a slot becomes a
 * chord. That separation is worth keeping precisely so this can leave again
 * cleanly.
 */

import { fromPerceptual, toPerceptual } from "@amplib/color";
import { parseSlotList } from "./parse-slot-list";
 ...
 * opaque on purpose: deciding what a hue *means* belongs to the caller, and
 * baking one meaning in here would rule the others out.
 *
